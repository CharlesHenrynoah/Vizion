...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Créer le projet avec Supabase
      const { data: newProject, error } = await supabaseAdmin
        .from('projects')
        .insert({
          name,
          description: description || '',
          owner_id: session.user.id.toString(), // Conversion en chaîne pour éviter les problèmes de type
          color: color || '#3b82f6',
          icon: icon || 'clipboard',
          is_active: true,
      ...
      console.error('Erreur de base de données lors de la création du projet:', dbError);
      return NextResponse.json({ error: 'Erreur lors de la création du projet' }, { status: 500 });
    }
  } catch (error) {
    console.error('Erreur serveur lors de la création du projet:', error);
